...
These decisions lock the implementation choices required before product code starts. They refine the architecture, protocol, security, and operations docs without replacing them.

## Java Transport

- SSH/SFTP library: Apache MINA SSHD client, using `sshd-core` and `sshd-sftp`.
- Rationale: actively maintained Java SSH stack, supports SSH exec channels and SFTP in one dependency family, is testable with embedded/containerized SSH fixtures, and avoids shelling out to `ssh`.
...
- The CLI backend must deny or avoid dangerous dot commands and must not enable `ATTACH`, `.shell`, `.system`, `.read`, `.open`, `.load`, or `VACUUM INTO` unless a later explicit policy implements safe path authorization.
...
- Frame limit: default `maxFrameBytes=1048576`; peers negotiate limits in `hello`/`helloAck`; limits may be configured downward and only raised by explicit configuration.
...
- Result streaming: production must avoid unbounded JVM buffering. CSV parsing should be streamed or bounded for large results.
...
- Pooling contract: built-in pooling is future-only, explicit, partitioned by full security context, and capped at `pool.maxSize=5` by default.

## SQL And JDBC Semantics

- Read/write mode: `readonly=true` opens the CLI with `-readonly`, sets `PRAGMA query_only=ON` where supported, disables dangerous dot commands, and rejects writes before execution where detectable. `readonly=false` runs `sqlite3` without `-readonly` and allows write SQL through stdin. Strong authorizer enforcement requires an optional helper or another direct SQLite API path.
- Standard JDBC parameters: `PreparedStatement` setters bind by SQLite parameter index only after the implementation provides true binding. Until then parameter binding must be implemented with documented CLI-safe behavior, such as safe SQL literal rendering or controlled temporary parameter commands; values must never be shell-interpolated.
- Named parameters: optional non-standard protocol `namedParams` binds by exact SQLite parameter name including prefix. Mixing `params` and `namedParams` in one request is rejected for MVP.
- SQLite shell `.param`: disabled by default. If `param.dotCommands=true` is implemented later, it is a compatibility preprocessor for a limited `.param set/list/clear` subset only; `.open`, `.read`, `.shell`, `.system`, `.load`, and arbitrary dot commands remain unsupported.

...
Production read-only MVP support:

| Area | Methods and behavior |
| --- | --- |
| Driver | `Driver.connect()`, `acceptsURL()`, basic property metadata |
| Connection | `createStatement()`, `getAutoCommit()`, `close()`, `isClosed()`, `isValid()`, `getMetaData()`, readonly/schema/catalog methods needed by tools |
| Statement | `executeQuery(sql)`, read-only `execute(sql)`, `close()`, `getResultSet()`, `getUpdateCount()`, `setMaxRows()`, `setFetchSize()`, connection-fatal `setQueryTimeout()` behavior where implemented |
| ResultSet | Forward-only `next()`, typed getters, `getObject()`, `getBytes()`, `wasNull()`, close |
| Metadata | Basic `ResultSetMetaData` and enough `DatabaseMetaData` for DBeaver/DataGrip browsing |

Post-read-only milestones:

| Milestone | Adds |
| --- | --- |
| PreparedStatement milestone | `prepareStatement()`, positional setters, `clearParameters()`, execute methods |
| Write-capable release | `executeUpdate(sql)`, mutating `execute(sql)`, `setAutoCommit()`, `commit()`, `rollback()`, data-edit write policy |
...
- Write-capable JDBC behavior until the write release.
- PreparedStatement support until its milestone.
- BLOB streaming through `InputStream`.
- Advanced generated-keys behavior beyond SQLite `last_insert_rowid()`.
- Built-in pooling, deferred to a future max-5 `DataSource` release.
...
- Transactions are reported unsupported for the production read-only MVP because manual transaction methods are not exposed; the write-capable release reports transaction support only with implemented `setAutoCommit`, `commit`, and `rollback` semantics.
- Multiple open results are unsupported in MVP.
...
| `getDatabaseProductVersion()` | Remote SQLite version from helper |
...
Data-grid edit support must default to non-writable metadata unless safe row identity is proven. Mark editable only for tables with an explicit `INTEGER PRIMARY KEY` rowid alias or complete non-null primary-key metadata. Rowid-only editing remains unsupported unless pinned DataGrip/DBeaver evidence proves safe exposed `_rowid_` identity handling. Views, virtual tables, FTS tables, generated/hidden columns, `WITHOUT ROWID` tables without full primary-key metadata, internal `sqlite_%` tables, and readonly connections are non-editable by default.

## Future Pooling Semantics

The built-in pool is a future `DataSource` feature, not hidden behavior inside plain `Driver.connect()`. It is not part of production read-only MVP.
...
- Pools are keyed by the full security context and immutable open options, including SSH identity, host-key policy, `sqlite3.path`, database path, readonly mode, output mode, protocol version, and capabilities. Optional helper mode additionally keys on helper path/hash/version.
...
- `getMoreResults()` returns `false` for MVP unless multi-result execution is added.
- `executeQuery()` must reject SQL that does not return rows.
- `executeUpdate()` must reject SQL that returns rows.
- Multiple SQL statements in one call must be rejected for MVP. The helper must use SQLite prepare-tail validation and reject non-whitespace trailing SQL before execution.
...
- Re-prepare on each execution for MVP unless server-side prepared statement IDs are later added.
...
- It strips recognized dot-command lines before sending SQL to the helper.
- It stores parameters in a per-statement map by default.
- It sends values to the helper as `namedParams`, not by rewriting SQL text.
...
- `getBytes()` returns `byte[]` for BLOB values.
...
- MVP should store `Date`, `Time`, and `Timestamp` as ISO-8601 text unless configured otherwise.
...
The production read-only MVP does not expose manual JDBC transactions. `getAutoCommit()` returns `true`; `setAutoCommit(boolean)`, `commit()`, and `rollback()` throw `SQLFeatureNotSupportedException`; and `DatabaseMetaData.supportsTransactions()` returns `false`.

The rules below apply to the write-capable release that enables manual transaction control.
...
- `BEGIN IMMEDIATE` is optional through `transactionMode=immediate` or explicit write intent.
- `BEGIN EXCLUSIVE` requires explicit opt-in.
...
- The driver must not mark a transaction committed until the helper confirms SQLite completed `COMMIT` successfully.
- The driver must not mark a transaction rolled back until the helper confirms SQLite completed rollback successfully.
...
- If SSH EOF, helper crash, protocol failure, or timeout happens after a mutating autocommit statement or `COMMIT` was sent but before success was received, the mutation outcome is unknown to JDBC and must not be retried automatically.
...
| Helper dies mid-transaction | Connection broken; transaction outcome unknown unless SQLite rollback on close is certain |
| Close with active transaction | Attempt rollback, then close helper/channel |
...
- `SQLITE_FULL`, `SQLITE_IOERR_*`, `SQLITE_CORRUPT`, `SQLITE_NOTADB`, timeout, helper crash, SSH EOF, or protocol failure: transaction outcome is unknown, connection is broken/tainted, and no automatic retry is allowed.
...
- Writes can block or fail if GMod or another client holds the writer lock.
...
- The helper must not change `journal_mode`, `synchronous`, `locking_mode`, or other durability-affecting PRAGMAs implicitly.
...
Possible pragmas for a read-mostly GMod deployment, only if the operator accepts the durability tradeoff and the application supports them:
...
Schema changes are disabled by default for MVP write sessions.
...
- `allowSchemaChanges` and `adminSql` are reserved for optional policy/helper backends. Canonical CLI mode allows normal sqlite3 scripting when `readonly=false`; operators are responsible for backups before running destructive SQL.
...
| `Statement.setQueryTimeout()` | Total statement runtime | Requires helper deadline and interrupt support |
...
- Set SQLite busy timeout per helper connection.
...
- If cancellation cannot be implemented safely, query timeout must terminate the helper and mark the connection broken.
- If helper termination happens while a transaction may be active, transaction outcome is unknown, the connection is broken, and pools must evict the physical connection without reuse.

## Metadata Queries

Use `sqlite_master` for table/view discovery. `sqlite_schema` is only an alias in newer SQLite releases and is not available on SQLite 3.27.2, which is a supported remote CLI version.
...
MVP generated-key behavior is explicit:
